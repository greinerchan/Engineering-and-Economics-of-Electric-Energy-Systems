...
while 1
    ...
    Vm = abs(Vtheta_Cur);
    delta = angle(Vtheta_Cur);
    Ym = abs(Y_bus); t = angle(Y_bus); 
    Jac_Reduced = makeJac(Ym,t,Vm,delta,obj_circ);
    ...
    if max(abs(delta_PQ(:,1))) < accuracy || iter > 2
        ...
        obj_circ_copy.gen(:,2) = abs(P_Gen(genList,1)) * baseMVA;
