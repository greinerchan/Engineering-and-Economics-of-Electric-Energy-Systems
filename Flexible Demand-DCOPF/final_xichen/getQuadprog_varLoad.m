function [H,f,A,b,Aeq,beq,LB,UB] = getQuadprog_varLoad(caseName)
...
A_1 = mpc.gencost(:,5); B_1 = mpc.gencost(:,6); 
A_2 = mpc.load(:,5); B_2 = mpc.load(:,6);
genLocate = mpc.gen(:,1);
loadLocate = mpc.load(:,1);
A1 = zeros(3 * busRow,3 * busRow);
B1 = zeros(1,3 * busRow);
for i = 1:1:length(genLocate)
    A1(genLocate(i),genLocate(i)) = A_1(i);
    B1(genLocate(i)) = B_1(i);
end
for i = 1:1:length(loadLocate)
    A1(busNum + loadLocate(i),busNum + loadLocate(i)) = A_2(i);
    B1(busNum + loadLocate(i)) = B_2(i);
end
...
for i = 1:1:length(loadLocate)
    P_d1(loadLocate(i),1) = P_d_low(i)/baseMVA;
    P_d2(loadLocate(i),1) = P_d_high(i)/baseMVA;
end
...
beq = zeros(busNum,1); LB = [LB_gen;P_d1;LB_Theta]; UB = [UB_gen;P_d2;UB_Theta];
